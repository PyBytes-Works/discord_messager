...
from keyboards import cancel_keyboard, users_keyboard
...
#         await message.answer(f'{user_name} назначен администратором. ', reply_markup=start_stop_keys())
...
#         await message.answer(f'Список команд администратора: {admin_commands}', reply_markup=start_stop_keys())
#     else:
#         logger.info(f'{message.from_user.id}:{message.from_user.username}: NOT AUTORIZATED')
#
#
...
#         f"Токен для нового пользователя {name}: {new_token}", reply_markup=start_stop_keys())
#     await state.finish()
#
#
# @logger.catch
...
#         f"Пользователь {name}: {user_id} удален из БД.", reply_markup=start_stop_keys())
#     await state.finish()
